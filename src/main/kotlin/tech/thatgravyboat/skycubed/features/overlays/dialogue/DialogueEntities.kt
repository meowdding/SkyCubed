...
object DialogueEntities {
    ...
    fun get(name: String, npc: DialogueNpc): LivingEntity? {
        val entity = lastClickedEntities.keys.find { npc -> McLevel.self?.getEntitiesOfClass(ArmorStand::class.java, npc.boundingBox)?.any { it.customName?.stripped == name } == true }
