...
object CurrencyInfo : InfoProvider {
    ...
    override fun getDisplay() = DisplayFactory.horizontal {
        if (SkyBlockIsland.THE_RIFT.inIsland()) {
            ...
            textDisplay(CurrencyAPI.motes.format(), shadow = true) { color = 0xFF55FF }
            return@horizontal
        }
        if (SkyBlockIsland.GARDEN.inIsland() or SkyBlockAreas.FARMHOUSE.inArea()) {
            display(getIconDisplay(SkyCubed.id("info/icons/sowdust")))
            textDisplay(CurrencyAPI.sowdust.format(), shadow = true) { color = TextColor.DARK_GREEN }

            display(getIconDisplay(SkyCubed.id("info/icons/bronze")))
            textDisplay(CurrencyAPI.copper.format(), shadow = true) { color = TextColor.RED }
        }

        display(getIconDisplay(SkyCubed.id("info/icons/purse")))
        textDisplay(CurrencyAPI.purse.format(), shadow = true) { color = TextColor.GOLD }

        display(getIconDisplay(SkyCubed.id("info/icons/bits")))
        textDisplay(CurrencyAPI.bits.format(), shadow = true) { color = TextColor.AQUA }
    }
}
