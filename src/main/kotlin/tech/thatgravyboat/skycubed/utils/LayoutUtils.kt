...
abstract class SkyCubedScreen(title: Component = CommonComponents.EMPTY) : BaseCursorScreen(title) {
    ...
    fun LayoutElement.applyLayout() {
        this.visitWidgets {
            it.isFocused = true
            this@SkyCubedScreen.addRenderableWidget(it)
        }
    }
    ...
    fun LayoutElement.applyAsRenderable() {
        this.visitWidgets {
            it.isFocused = true
            this@SkyCubedScreen.addRenderableOnly(it)
        }
    }

    fun LayoutElement.center() = this.apply {
        FrameLayout.centerInRectangle(this, 0, 0, this@SkyCubedScreen.width, this@SkyCubedScreen.height)
        if (this is Layout) {
            this.arrangeElements()
        }
    }

    fun Layout.asScrollable(
        width: Int,
        height: Int,
